...
/**
 * Provides a resource to get reports by all organisations assigned to user
 *
 * @RestResource(
 *   id = "get_report_full_list",
 *   label = @Translation("Get all reports"),
 *   uri_paths = {
 *     "canonical" = "/api/v1/report/full-list"
 *   }
 * )
 */
class ReportFullListResource extends ResourceBase {
  ...
  public function get() {
    ...
    $organisations = IpUserDataController::getOrganisations($this->currentUser);

    if (is_array($organisations) && count($organisations)) {
      // get reports requests list by organisation id
      // @TODO: Group by report_node_id
      $reports_request = IpReportsRequestsModel::getAllByOrganisationListID(array_column($organisations, 'id'));
    }
    else {
    ...
    if ($reports_request && is_array($reports_request)) {
      ...
      foreach ($reports_request as $report_request) {
        ...
        $organisation_title = NULL;

        if (is_array($organisations) && count($organisations)) {
          $org_indx = array_search($report_request->organisation_id, array_column($organisations, 'id'));

          if ($org_indx !== FALSE) {
            $organisation_title = $organisations[$org_indx]['title'];
          }
        }
        ...
        $response_report = [
          ...
          'organisation_id'    => $report_request->organisation_id,
